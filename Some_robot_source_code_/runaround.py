  ...
  for innercount in range(MAX_INNERCOUNT):
    ...
    if (FOLLOW_YELLOW_GREEN and not(following_line)):
      ...
      and (color == GREEN or color == YELLOW)):
        ...
        motorR.run_forever(speed_sp= -SLOW_TURN_SPEED)
        motorL.run_forever(speed_sp= -SLOW_TURN_SPEED)
        ...
        if (color != YELLOW and color != GREEN):
          ...
          # Hit second of two colors 
            ...
            or (color == YELLOW and not(HEAD_SOUTH))):
              ...
              sign = 1
            motorR.run_forever(speed_sp=  sign * SLOW_TURN_SPEED)
            motorL.run_forever(speed_sp= -sign * SLOW_TURN_SPEED)
    ...
    if (FOLLOW_YELLOW_GREEN and following_line):
      ...
      (color == GREEN and not(HEAD_SOUTH))):
        motorR.run_forever(speed_sp= -WIGGLE_SPEED*WIGGLE_FACTOR)
        motorL.run_forever(speed_sp= -WIGGLE_SPEED)
      if ((color == GREEN and HEAD_SOUTH) or
      (color == YELLOW and not(HEAD_SOUTH))):
        motorR.run_forever(speed_sp= -WIGGLE_SPEED)
        motorL.run_forever(speed_sp= -WIGGLE_SPEED*WIGGLE_FACTOR)
    ...
    (not(CROSS_RED_BLUE) and (color == RED or color == BLUE)))):
      motorR.run_forever(speed_sp=  FAST_TURN_SPEED)
      motorL.run_forever(speed_sp= -FAST_TURN_SPEED)
      sleep(TURN_DELAY)
