  ...
  for innercount in range(MAX_INNERCOUNT):
    ...
    if color == BLACK or color == UNKNOWN:
      ...
      if LANE == 'r':
        ...
        motorFR.run_forever(speed_sp=-(MAX_SPEED + FAST_TURN_SPEED))
        motorBR.run_forever(speed_sp=(MAX_SPEED + FAST_TURN_SPEED))
        motorBL.run_forever(speed_sp=(MAX_SPEED - FAST_TURN_SPEED))
        motorFL.run_forever(speed_sp=-(MAX_SPEED - FAST_TURN_SPEED))
        sleep(1.5)
        motorFR.run_forever(speed_sp=(MAX_SPEED - FAST_TURN_SPEED))
        motorBR.run_forever(speed_sp=-(MAX_SPEED - FAST_TURN_SPEED))
        motorBL.run_forever(speed_sp=-(MAX_SPEED + FAST_TURN_SPEED))
